...
|---|---|---|---|---|---|---|
| `mha_b8_n4096_h16` (B=8, N=4096, H=16, D=128, causal, bf16, layout (B,N,H,D)) | Opus-5 | `mha_b8_n4096_h16/opus-5/cute_bwd.py` (CuTe DSL + Triton) | 1398.0 µs | 1394.1 µs | 0.997x | 30 h |
...
Entry points: `attention_backward(q, k, v, o, lse, do, scale)` (Opus-5 MHA);
...
- Correctness was checked against an fp32 naive implementation (forward written out,
  ...
  relative-L2 error against that same naive. All five pass on fresh seeds.
...
- MHA Fable-5.1 (0.947x so far) is not included; that session is still running.
